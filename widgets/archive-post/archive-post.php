...
class Easyel_Archive_Post__Widget extends \Elementor\Widget_Base {
    ...
    public function get_title() {
        return esc_html__( 'Easy Archive posts', 'easy-elements' );
    }
    ...
    protected function register_controls() {
        ...
        $this->add_control(
            'active_number_title',
            [
                'label' => __( 'Active Number', 'easy-elements' ),
        ...
        $this->add_control(
            ...
            [
                ...
                'type' => \Elementor\Controls_Manager::COLOR,
                'selectors' => [
                    '{{WRAPPER}} .eel-blog-pagination ul li span.current, {{WRAPPER}} .eel-blog-pagination ul li a:hover' => 'color: {{VALUE}};',
                ],
                'condition' => [
                    'show_pagination' => 'yes',
                ],
    ...
    protected function render() {
        ...
        $args = [
            'post_type' => $post_type,
            'posts_per_page' => $settings['posts_per_page'],
            'orderby'        => ( class_exists('Easy_Elements_Pro') && !empty($orderby) ) ? $orderby : 'date',
            'order'          => ( class_exists('Easy_Elements_Pro') && !empty($order) ) ? $order : 'DESC',
        ];
        ...
        // Use global query on archive, home, or search pages
        ...
        if ( $query->have_posts() ) :
                                ...
                                <a href="<?php the_permalink(); ?>">
                                    <?php
                                        if ( has_post_thumbnail() ) {
                                            ...
                                            the_post_thumbnail( $thumbnail_size, array( 'alt' => $alt ) );
                                        }
                                    ?>
                                </a>
                            </div>
            ...
            ?>
            <div class="eel-no-posts-found">
                <p><?php esc_html_e( 'Nothing Found', 'easy-elements' ); ?></p>
                <p><?php esc_html_e( 'Sorry, but nothing matched your search terms. Please try again with some different keywords.', 'easy-elements' ); ?></p>
                
                <div class="eel-search-form">
                    <form role="search" method="get" class="eel-search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
                        <input type="search" class="eel-search-field" placeholder="<?php esc_attr_e( 'Search...', 'easy-elements' ); ?>" value="<?php echo get_search_query(); ?>" name="s" />
                        <button type="submit" class="eel-search-submit">
